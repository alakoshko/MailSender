        ...
        private void SendMail(MailAddress emailFrom, MailAddress emailTo, string emailSubject, string emailText)
        {
            try
            {
                ...
                })
                {
                    ...
                    {
                        Trace.WriteLine( MailSender.GlobalSettings.mailBodyEmpty);
                        throw new InvalidOperationException(MailSender.GlobalSettings.mailTitleError, new Exception(MailSender.GlobalSettings.mailBodyEmpty));
                    }
            ...
            catch (Exception error)
            {
                ...
                throw new InvalidOperationException(MailSender.GlobalSettings.smtpErrorTittle, error);
            }
        }
    }
}
